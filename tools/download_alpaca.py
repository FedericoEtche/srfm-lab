"""
Download historical bars via Alpaca Markets API.
Free paper trading account — includes historical data back to 2016.
Saves to tools/data_cache/ as CSVs alongside the Twelve Data cache.
...
def fetch_bars(ticker: str, timeframe: str, start: str, end: str) -> pd.DataFrame:
    """
    timeframe: '1Hour' or '15Min'
    Returns DataFrame with columns Open/High/Low/Close/Volume, DatetimeIndex (ET, tz-naive)
    """
    all_bars = []
    params = {
        "start":      start,
    ...
    # Convert UTC to ET (approximate: -5h, close enough for bar alignment)
    df["datetime"] = df["datetime"].dt.tz_convert("America/New_York").dt.tz_localize(None)
    df = df.set_index("datetime")[["Open", "High", "Low", "Close", "Volume"]]
    df = df.sort_index()
    # Keep regular hours only
...
if __name__ == "__main__":
    ...
    for label, ticker in SYMBOLS.items():
        ...
        print()
    print("Done. Run local_backtest.py to backtest.")
